...
export const getRecentcomments = async (req, res, next) => {
  try {
    ...
    // Fetch comments and populate the postId field with the post's slug
    const comments = await Comment.find()
      .sort({ createdAt: sortDirection })
      .skip(startIndex)
      .limit(limit)
      .populate({
        path: "postId", // Field to populate
        select: "slug", // Only include the slug field from the Post model
      });
